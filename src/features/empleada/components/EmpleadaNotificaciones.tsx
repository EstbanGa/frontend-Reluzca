...
function EmpleadaNotificaciones() {
  ...
  const eliminarNotificacion = async (notificacionId: string) => {
    if (!confirm(t('empleada.notifications.confirmDelete'))) return;
  ...
  const formatearFecha = (fechaStr: string) => {
    ...
    if (minutos < 1) return t('empleada.notifications.time.justNow');
    if (minutos < 60) return t('empleada.notifications.time.minutes', { count: minutos });
    if (horas < 24) return t('empleada.notifications.time.hours', { count: horas });
    if (dias < 7) return t('empleada.notifications.time.days', { count: dias });
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <RefreshCw className="w-12 h-12 text-[#D95B26] animate-spin mx-auto mb-4" />
          <p className="text-gray-600">{t('empleada.notifications.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">{t('empleada.notifications.errorLoading')}</h2>
          <p className="text-gray-600 mb-4">{error}</p>
          <button
            onClick={fetchNotificaciones}
            className="px-6 py-2 bg-[#D95B26] text-white rounded-lg hover:bg-[#c04f20] transition-colors flex items-center gap-2 mx-auto"
          >
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <ListPageHeader
        ...
        title={t('empleada.notifications.title')}
        subtitle={t('empleada.notifications.subtitle')}
      ...
      {data && (
        <ListStatsGrid
          columns={3}
          stats={[
            { label: t('empleada.notifications.stats.total'), value: data.estadisticas.total, color: '#D95B26' },
            { label: t('empleada.notifications.stats.unread'), value: data.estadisticas.no_leidas, color: '#2563eb' },
            { label: t('empleada.notifications.stats.read'), value: data.estadisticas.leidas, color: '#16a34a' },
          ]}
        />
      )}
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg transition-colors ${
              filtro === "todas"
                ? "bg-[#D95B26] text-white"
                : "bg-white text-gray-700 hover:bg-gray-100"
            }`}
          >
            {t('empleada.notifications.filters.all')}
          </button>
          <button
          ...
          >
            {t('empleada.notifications.filters.unread')}
          </button>
          <button
          ...
          >
            {t('empleada.notifications.filters.read')}
          </button>
        </div>
        ...
        {data && data.estadisticas.no_leidas > 0 && (
          ...
          >
            ...
            {t('empleada.notifications.markAllRead')}
          </button>
        )}
      </div>
        ...
        {notificacionesFiltradas && notificacionesFiltradas.length > 0 ? (
          notificacionesFiltradas.map((notificacion) => {
            ...
            return (
              ...
              >
                <div className={`flex items-start gap-4 ${!notificacion.leida ? 'border-l-4 border-l-[#D95B26] -ml-3 pl-3' : ''}`}>
                      ...
                      <div className="mt-3 p-3 bg-gray-50 rounded-lg text-sm">
                        ...
                        {notificacion.reserva.plan && (
                          <div className="mt-2 text-gray-700">
                            <span className="font-medium">{t('empleada.notifications.planLabel')}</span> {notificacion.reserva.plan.nombre}
                          </div>
                        )}
                      </div>
                    )}
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">
              {t('empleada.notifications.empty.all')}
            </h3>
            <p className="text-gray-500">
              {filtro === "todas"
                ? t('empleada.notifications.empty.allDetail')
                : filtro === "no_leidas"
                ? t('empleada.notifications.empty.unread')
                : t('empleada.notifications.empty.read')}
            </p>
          </div>
        )}
      </div>
...
export default withEmpleadaRole(EmpleadaNotificaciones);