        ...
        {tab === "detalles" && (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { icon: Mail,     label: "Correo",     value: usuario.correo },
                { icon: Phone,    label: "Teléfono",   value: usuario.telefono ?? "No registrado" },
                { icon: User,     label: "Género",     value: usuario.genero ?? "No especificado" },
                { icon: Calendar, label: "Nacimiento", value: fmtDate(usuario.fecha_nacimiento) },
                { icon: MapPin,   label: "Dirección",  value: usuario.direccion ?? "No registrada" },
              ].map(({ icon: Icon, label, value }) => (
                <div key={label} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                  <Icon className="h-4 w-4 text-[#195083]/60 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs text-gray-500 font-medium">{label}</p>
            ...
            {!editing ? (
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 space-y-4">
                <h3 className="font-semibold text-[#195083] flex items-center gap-2">
                  <Edit3 className="h-4 w-4" />
                  Editando usuario
                </h3>
                {editError && (
