...
function ClienteUbicaciones() {
  ...
  const handleDelete = async (ids: string[]) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/usuario/clientes/ubicaciones/`, {
        method: 'DELETE',
        headers: {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/ubicaciones/cliente/${editingUbicacion.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
